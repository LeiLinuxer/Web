...
set "relcp=..\..\graalvm\launcher-common.jar"
set "absolute_cp="
set newline=^
...
if "%module_launcher%"=="True" (
  set "main_class=--module None/org.graalvm.launcher.PolyglotLauncher"
  ...
  set "main_class=org.graalvm.launcher.PolyglotLauncher"
  set "app_path_arg=-cp"
)

if "%VERBOSE_GRAALVM_LAUNCHERS%"=="true" echo on

"%location%..\..\..\bin\java"  %jvm_args% %app_path_arg% "%absolute_cp%" %main_class% %launcher_args%
...
:process_vm_arg
    ...
    if "!vm_arg!"=="cp" (
        ...
        set "part2='--%prefix%.cp=..\..\graalvm\launcher-common.jar', not two separate arguments."
        >&2 echo !part1! !part2!
        exit /b 1
    ) else if "!vm_arg!"=="classpath" (
        set "part1='--%prefix%.classpath' argument must be of the form"
        set "part2='--%prefix%.classpath=..\..\graalvm\launcher-common.jar', not two separate arguments."
        >&2 echo !part1! !part2!
        exit /b 1
...
:process_arg
    ...
    if "!arg:~0,5!"=="--vm." (
        ...
        if "!arg!"=="--native" (
            ...
            set "part2='native-image --macro:polyglot-launcher!extra!'."
            >&2 echo !part1! !part2!
            exit /b 1
        ) else (
